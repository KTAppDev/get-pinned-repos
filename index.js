...
export async function getPinnedRepos(username) {
  ...
  return $(".js-pinned-item-list-item")
    .toArray()
...
export async function getPinnedRepos_v2(username) {
  ...
  // Select and extract href attributes of pinned repository links.
  const pinnedRepos = $(".js-pinned-item-list-item").toArray();

  // get the repo names
  const repos = pinnedRepos.map((el) =>
    $(el).find("a").attr("href").replace(/^\//, ""),
  ...
  const stars = pinnedRepos.map((el) => {
    const starsElement = $(el)
      .find("p")
      .toArray()
      .map((el) => $(el).text().split(",")[0].split(","))[1];
